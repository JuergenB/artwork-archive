...
- **Duplicate artwork records in the export set (found 2026-07-30, unresolved).** `Approved for Export` holds **123 records but only 117 distinct artworks** — 6 surplus rows that would ship to AA twice. Two causes: Beth Grossman double-submitted 78s apart (2026-07-06 18:33:07 and 18:34:25 — 4 pieces), while O'Malley Ceramics and Johny Chang each have two records with an *identical* `Date Imported`, i.e. one submission written twice by intake. Detect with: group `Approved for Export` on (`Piece Name`, `Artist Email`) and count groups > 1. Deduping is a curator decision — do not delete records unprompted.
